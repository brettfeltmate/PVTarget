...
class Cache(EnvAgent):
	...
	def write(self, store, data, timestamp=None):
		...
		self.__store[store].append([self.exp.target.x if data is None else data, now() if not timestamp else timestamp])
...
class Accelerator(EnvAgent):
	...
	def gain(self):
		if not self.active:
			...
			if not self.active:
				cso("\t\t<blue_d>Acceleration ended</blue_d> goal: {0}s actual: {1}s".format(self.duration, now() - self.start))
				self.__rate = 0
				self.duration = None
				self.start = None
				self.accumulated_v = []
	...
	def reset(self):  # method exists for readability basically; the actual resetting is done in the (if) cond in apply()
		self.active = False
...
class PVTarget2(EnvAgent):
	...
	def __init__(self):
		...
		self.cfg = {'mouse_input': True, 'acceleration': False, 'track_input': True, 'poll_while_moving': True,
					'poll_at_fixation': False, 'exp_duration': 300, 'trial_count': 25,
					'trial_interval_bounds': [10, 15], 'reset_target_after_poll': True}
		...
		for event in event_queue:
			if event.type == SDL_KEYDOWN:
				...
				if key.sym is sdl2.keycode.SDLK_SPACE:
					self.cache.write(RESPONSE, now())
					self.polling = False
					if self.cfg['reset_target_after_polling']:
						self.x = P.screen_c
	...
	def refresh(self, event_queue):
		try:
			...
			next_poll_time = sum(self.intervals[0:self.cache.len(RESPONSE)]) if self.cache.len(RESPONSE) else self.intervals[0]
	...
	def __change_direction(self, update_direction):
		...
		and we also want to ensure that the user input can't so overpower the target that a jerky handmotion can
	...
	def __refresh(self, event_queue):
		...
		if self.cfg['poll_while_moving'] or not self.polling:  # ie. don't do this if both are True
			...
			if self.cfg['mouse_input']:
				...
				try:
					# "new velocity  is the current velocity plus a value for mouse velocity expressed as a fraction of it's
					# inverse power function; jon doesn't know this math, it just means that as mouse velocity gets bigger
			...
			try:
				...
				# similarly, if velocity falls below 1px/s, treat this as essentially zero velocity & do a direction change
