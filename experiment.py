...
# from ExpAssets.Resources.code.PVTarget2 import PVTarget2
...
class PhysicsEngine(klibs.Experiment):
    ...
    def setup(self):
        ...
        self.pvt_machine.cfg['exp_duration'] = 150  # seconds
        self.pvt_machine.cfg['trial_count'] = 9
        ...
        # self.pvt_machine.cfg['track_input'] = False  # decides whether cursor is visible
        ...
        hide_mouse_cursor()
        session_start = now()
        while (now() - session_start) < self.pvt_machine.cfg['exp_duration']:
            events = pump(True)
            self.pvt_machine.refresh(events)
            ui_request(queue=events)

        self.pvt_machine.cache.dump(to_file=True)
        self.db.export(table='mouse_movement')
        self.db.export(table='target_position')
        self.db.export(table='mouse_velocity')
        self.db.export(table='target_velocity')
        self.db.export(table='target_direction')
        self.db.export(table='pvt_response')
        exit()
        # -----------------------------------------!!! TEST  ENDS  HERE !!! -----------------------------------------
